...
<div>
    ...
    <div class="mb-5" x-data="{ isOpen: @if ($openFilter || request('openFilter')) true @else false @endif }">
        ...
        <button type="button" @click="isOpen = !isOpen"
            class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs leading-4 font-medium rounded-t text-grey-700  hover:bg-grey-300 bg-gray-700 text-gray-200 transition ease-in-out duration-150">
        ...
        <a href="{{ route('admin.settings.sent-emails') }}"
            class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs leading-4 font-medium rounded text-grey-700  hover:bg-grey-300 bg-gray-700 text-gray-200 transition ease-in-out duration-150">
            <svg class="w-5 h-5 text-gray-200" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <main class="flex flex-1 bg-gray-200">
        ...
        <div class="relative flex flex-col flex-grow w-full max-w-xs bg-gray-200 border-l border-r">
            <div class="flex-1 overflow-y-auto">
                @foreach ($this->emails() as $email)
                    <a href="#" wire:click="view({{ $email->id }})"
                        class="block px-6 pt-3 pb-4 bg-white border-t emailitem">
                        <div class="flex justify-between">
